...
RakuFinance is a module one can use to calculate various data for a portfolio of investments
...
data in a comma-separated-value file with the appropriate header line (see B<Input Files>)
and historical data available from public data sources.
...
for security XXX (number of shares, prices, and other transacion datate
are not shown).
...
segregated into another directory outside of the working 
directory. The absolute path of that directory should 
be defined in environment variable B<RAKUFINANCE_USER_DATA>.
